        ...
        HasEvents(
            HasGlobalScopes(
                HasRelationships(
                    HasTimestamps(
                        HidesAttributes(
                            GuardsAttributes(ForwardsCalls(class {}))
                        )
                    )
                )
            )
        ...
        UrlRoutable
{
    ...
    */
    ...
    protected bootIfNotBooted(): void {
        if (
            !isset(
                (<typeof Model>this.constructor).$booted[this.constructor.name]
            )
        ) {
            (<typeof Model>this.constructor).$booted[this.constructor.name] =
                true;
    ...
    public replicate($except: any[] | null = null): this {
        ...
        return tap(
            new (<typeof Model>this.constructor)(),
            function ($instance) {
    ...
    public resolveChildRouteBinding($childType, $value, $field): Model | null {
        ...
        if (
            ...
            return $relationship
                .where(
                    $relationship.getRelated().getTable() + "." + $field,
                    $value
                )
                .first();
        } else {
