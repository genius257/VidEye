    ...
    Object.keys(watched).forEach((series) =>
      Object.keys(watched[series]).forEach((season) =>
        Object.keys(watched[series][season]).forEach((episode) => {
          let YTID = this.state.series[series].seasons[season][episode].YTID;
          ...
          jobs.push(
            fetch(
              `https://www.googleapis.com/youtube/v3/videos?id=${YTID}&part=contentDetails&key=${config.apiKey}`
            )
              ...
              .then((json) =>
                result.push({
                  ...
                  season,
                  episode,
                  time: watched[series][season][episode]["time"],
                  totalTime: Math.max(
                    moment
                      .duration(json?.items?.[0]?.contentDetails?.duration ?? 0)
                      .asSeconds(),
                    watched[series][season][episode]["duration"]
                  )
                })
              )
          );
                  ...
                  <Card
                    image={
                      ...
                      `url('https://img.youtube.com/vi/${
                        this.state.series[watch.series].seasons[watch.season][
                          watch.episode
                        ].YTID
                      }/mqdefault.jpg')`
                    }
          ...
          {Object.keys(this.state.series).map((key) => (
            <Link to={`/series/${key}/`} key={key}>
              <Poster
                ...
                image={"url('" + this.state.series[key].image + "')"}
              />
            </Link>
          ))}
        </div>
